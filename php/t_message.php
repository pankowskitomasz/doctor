...
class TMessage{
    ...
    private function checkMessagePresence($firstA,$lastA,$phoneA,$emailA,$msgA,$consultA){
        $sql = "select id from ";
        $sql .= DB_PREFIX;
        $sql .= "messages where ";
    ...
    public function insert($firstA,$lastA,$phoneA,$emailA,$msgA,$consultA){
        if(!$this->checkMessagePresence($firstA,$lastA,$phoneA,$emailA,$msgA,$consultA)){
            $sql = "insert into ";
            $sql .= DB_PREFIX;
            $sql .= "messages(firstname,lastname,phone,email,consultation,message) ";
            $sql .= "values(:fn,:ln,:ph,:em,:cons,:msg)";
            $stmt = $this->link->prepare($sql);
            $stmt->bindParam(":fn",$firstA);
            $stmt->bindParam(":ln",$lastA);
            $stmt->bindParam(":ph",$phoneA);
